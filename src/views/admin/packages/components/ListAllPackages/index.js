...
import { getAllPackages } from 'crud'
...
export default function ListAllPackages({ reload, setReload, setRender }) {
                ...
                <Table className={classes.table} aria-label="simple table">
                    <TableHead style={{ backgroundColor: 'black' }}>
                        <TableRow>
                            ...
                            <TableCell style={{ color: 'white' }} align="right">Origin</TableCell>
                            <TableCell style={{ color: 'white' }} align="right">Destination</TableCell>
                    ...
                    <TableBody className='position-relative'>
                        {listPackages.length === 0 ? (
                            ...
                            listPackages.map((pkg) => (
                                <TableRow className={classes.tr} onClick={() => setRender(pkg._id)} key={pkg._id}>
                                    <TableCell component="th" scope="row">
                                        <ListItem className='pl-0'>
                                            <ListItemAvatar>
                                                <Avatar alt={pkg.name} src={pkg.featuredImg} />
                                            </ListItemAvatar>
                                            <ListItemText
                                                primary={pkg.name}
                                    ...
                                    <TableCell align="right">
                                        <Chip color="primary" size="small" label={pkg.origin} />
                                    </TableCell>
                                    <TableCell align="right">
                                        <Chip color="secondary" size="small" label={pkg.destination} />
                                    </TableCell>
                                    <TableCell align="right">
                                        <Chip size="small" label={pkg.price} />
                                    </TableCell>
                                    <TableCell align="right">
                                        {pkg.status === 'INACTIVE' ?
                                            <Chip size="small" style={{ backgroundColor: 'red', color: 'white' }} label={pkg.status} /> :
                                            <Chip size="small" style={{ backgroundColor: 'green', color: 'white' }} label={pkg.status} />
                                        }
                                    </TableCell>
                                    <TableCell align="right">
                                        <Chip size="small" label={
                                            <Moment format="DD/MM/YYYY">{pkg.createdAt}</Moment>
                                        } />
                                    </TableCell>
                                </TableRow>
                            )))}
