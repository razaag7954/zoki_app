...
export default function ListAllHotels({ reload, setReload, setRender }) {
    ...
    const fetchData = () => {
        ...
        getAllHotels(params)
            .then(res => {
                ...
                setListHotels(res.data.data.hotels)
    ...
    return (
        <div>
            <div className='d-flex w-100 mb-1 justify-content-between align-items-center'>
                <h5 className='p-0 m-0'>All Hotels</h5>
                    ...
                    <TableBody className='position-relative'>
                        {listHotels.length === 0 ? (
                            ...
                            listHotels.map((hotel) => (
                                <TableRow className={classes.tr} onClick={() => setRender(hotel._id)} key={hotel._id}>
                                    <TableCell component="th" scope="row">
                                        <ListItem className='pl-0'>
                                            <ListItemAvatar>
                                                <Avatar alt={hotel.name} src={hotel.featuredImg} />
                                            </ListItemAvatar>
                                            <ListItemText
                                                primary={hotel.name}
                                                secondary={`${hotel.address.city}, ${hotel.address.country}`}
                                            />
                                        </ListItem>
                                    </TableCell>
                                    <TableCell align="right">
                                        <Chip color="primary" size="small" label={hotel.contactInfo.mobile} />
                                    </TableCell>
                                    <TableCell align="right">
                                        <Chip color="secondary" size="small" label={hotel.contactInfo.email} />
                                    </TableCell>
                                    <TableCell align="right">
                                        {hotel.status === 'INACTIVE' ?
                                            <Chip size="small" style={{ backgroundColor: 'red', color: 'white' }} label={hotel.status} /> :
                                            <Chip size="small" style={{ backgroundColor: 'green', color: 'white' }} label={hotel.status} />
                                        }
                                    </TableCell>
                                    <TableCell align="right">
                                        <Chip size="small" label={
                                            <Moment format="DD/MM/YYYY">{hotel.createdAt}</Moment>
                                        } />
                                    </TableCell>
                                </TableRow>
                            )))}
