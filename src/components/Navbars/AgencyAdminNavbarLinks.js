...
export default function AgencyAdminNavbarLinks({ agencyId }) {
  ...
  React.useEffect(async () => {
    if (agencyId) {
      try {
        const wallet = await getSpecificWallet(agencyId)
  ...
  const viewUserWalletHandler = () => {
    dispatch(setLoading())
    const layout = location.pathname.split('/')[1]
    history.push(`/${layout}/user-wallet`)
    dispatch(unsetLoading())
    setOpenProfile(null);
  }
